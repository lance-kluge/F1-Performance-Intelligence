...
def test_tire_figures_preserve_uncertainty_and_validation_contract(tire_analysis_run) -> None:
    ...
    assert rates.layout.xaxis.tickmode == "array"
    assert all(len(label.rpartition(".")[2]) == 3 for label in rates.layout.xaxis.ticktext)
...
def test_degradation_rate_axis_uses_clean_explicit_tick_labels(tire_analysis_run) -> None:
    analysis = tire_analysis_run.analysis
    kimi_medium = replace(
        analysis.estimates[0],
        seconds_per_lap=-0.40570713631307065,
        confidence_lower_seconds_per_lap=-0.5995769046496504,
        confidence_upper_seconds_per_lap=-0.21183736797649094,
    )
    figure = degradation_rate_figure(replace(analysis, estimates=(kimi_medium,)))

    assert list(figure.layout.xaxis.tickvals) == [-0.6, -0.4, -0.2, 0.0]
    assert list(figure.layout.xaxis.ticktext) == ["-0.600", "-0.400", "-0.200", "+0.000"]
    assert all("0000000000001" not in label for label in figure.layout.xaxis.ticktext)
